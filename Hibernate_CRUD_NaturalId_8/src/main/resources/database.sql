...
insert into owner_table (first_name, last_name, gender, city, state, mobile_number, email_id, pet_id, pet_name, pet_date_of_birth, pet_gender, pet_type) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
...
select * from owner_table where email_id = ?;
